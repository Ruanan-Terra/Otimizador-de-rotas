...
def abre_rotas():
    while len(driver.find_elements(By.CLASS_NAME, 'm6QErb')) < 1:
        sleep(1)

    driver.find_element(By.CLASS_NAME, 'etWJQ').click()

    if driver.find_element(By.CLASS_NAME, 'ExQYxb'):
        print('Abriu rotas')
...
def retorna_tempo_total():
    ...
    elemento_tempo = driver.find_element(By.XPATH, xpath)
...
def retorna_distancia_total():
    ...
    wait = WebDriverWait(driver, timeout = 3)
    wait.until(EC.presence_of_element_located((By.XPATH, xpath)))
    elemento_distancia = driver.find_element(By.XPATH, xpath)
    return float(elemento_distancia.text.replace(',', '.').replace(' km', ''))

# ======================================== FUNÇÕES PRINCIPAIS ======================================== #

def gera_pares_distancia(pares):
    distancia_pares = {}
...
def gera_orimizacao(enderecos, distancia_pares):
...
if __name__ == '__main__':
    ...
    solucao = gera_orimizacao(enderecos, distancia_pares)
